...
If we had bigger proportion of the training data, it would mean that we could better fit the model. That would mean we would have lower bias. If the training data would be bigger, that means

če bi bil veči proportion training data, manjši bias, večji standard error, ker bo test size smaller, večja varianca.
manjša trainin množica (večji test set), večji bias. večja varianca,ker slabo nafitan model.
...
```
```{r}
ggplot(data.frame(diff_2), aes(x=diff_2)) + geom_density()
```

```{r}
ggplot(data.frame(diff_4), aes(x=diff_4)) + geom_density()

```
```{r}
ggplot(data.frame(diff_10), aes(x=diff_10)) + geom_density()

```
```{r}
ggplot(data.frame(diff_2010), aes(x=diff_2010)) + geom_density()

```
```{r}
ggplot(data.frame(diff_100), aes(x=diff_100)) + geom_density()

```
  več foldov, bolš oceniš, boljši estimate.
  ve
...
CV se dela samo na neodvisnih columnih za podatke! če bi imel dataset kjer bi bili stolpci odvisni ali pa zelo mali dataset.